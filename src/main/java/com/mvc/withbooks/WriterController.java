...
public class WriterController {
	...
	public String WriterEpisodeUpdate(HttpServletRequest req, @ModelAttribute EpisodeDTO dto) {
		...
		if(res>0) {
			req.setAttribute("msg", "에피소드 수정 성공!! 에피소드 목록 페이지로 이동합니다.");
			req.setAttribute("url", "writerEpisodeList");
		}else {
			req.setAttribute("msg", "에피소드 수정 실패!! 에피소드 목록 페이지로 이동합니다.");
			req.setAttribute("url", "writerEpisodeList");
		}
		return "message";
	}
	...
	public String WriterNovel(HttpServletRequest req, @ModelAttribute NovelDTO dto, @RequestParam int mnum) {
		...
		String msg = null, url = null;
		if(res>0) {
			msg = "소설 등록 성공!! 소설 목록 페이지로 이동합니다.";
			url = "writerNovelList";
		}else {
			msg = "소설 등록 실패 !! 소설 목록 페이지로 이동합니다.";
			url = "writerNovelList";
		}
		req.setAttribute("msg", msg);
		req.setAttribute("url", url);
	...
	public String WriterNovelUpdate(HttpServletRequest req, NovelDTO dto) {
		...
		String msg = null, url = null;
		if(res>0) {
			msg = "소설 수정 성공!! 소설 목록 페이지로 이동합니다.";
			url = "writerNovelList";
		}else {
			msg = "소설 수정 실패 !! 소설 목록 페이지로 이동합니다.";
			url = "writerNovelList";
		}
		req.setAttribute("msg", msg);
		req.setAttribute("url", url);
	...
	public String writerNoveldelete(HttpServletRequest req, @RequestParam int nnum) {
		...
		String msg = null, url = null;
		if(res>0) {
			msg = "소설 삭제 성공!! 소설 목록 페이지로 이동합니다.";
			url = "writerNovelList";
		}else {
			msg = "소설 삭제 실패 !! 소설 목록 페이지로 이동합니다.";
			url = "writerNovelList";
		}
		req.setAttribute("msg", msg);
		req.setAttribute("url", url);
